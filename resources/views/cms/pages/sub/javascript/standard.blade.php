<script type="text/javascript">
    ...
    $(".set-removability").on("change", function() {
        ...
        $.ajax({
            type:'POST',
            url:'/cms/pages/set-removability/'+id,
            headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}' },
    ...
    $( "input[name='title']" ).keyup(function() {
        $("input[name='url']").val( stringToSlug($(this).val()) + '-' + getDate() );
    });

    $("input[name='released_at']").on("change", function() {
        var title = $("input[name='title']").val();
        var slug = stringToSlug(title) + '-' + getDate();

        $("input[name='url']").val(slug);
    });

    function getDate() {
        return $("input[name='released_at']").val();
    }
